...
<div id="serials-goods-inventory" class="content">
    ...
    @if(!$serials->isEmpty())
        <div id="control-bar-serial-good" class="control-bar">
            ...
            <div class="control-actions">
                ...
                {{--
                    Acción masiva: aparece sola al seleccionar dos o más bienes.
                    Todas las tarjetas de esta vista son del mismo bien, así que
                    el mantenimiento siempre aplica a un solo tipo.
                --}}
                <button class="control-btn" type="button" data-serial-bulk hidden
                        title="Registrar mantenimiento a los bienes seleccionados"
                        onclick="event.stopPropagation(); btnRegistrarMantenimientoMasivo();">
                    <i class="fas fa-wrench"></i>
                    <span>Registrar mantenimiento</span>
                </button>
                @endif

                <button class="control-btn" type="button" data-serial-single title="Ver mantenimientos"
                ...
                <button class="control-btn" type="button" data-serial-single title="Dar de baja" onclick="event.stopPropagation(); if (typeof btnDarDeBajaBienSerial === 'function') btnDarDeBajaBienSerial();">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
                        <path d="M3.375 3C2.339 3 1.5 3.84 1.5 4.875v.75c0 1.036.84 1.875 1.875 1.875h17.25c1.035 0 1.875-.84 1.875-1.875v-.75C22.5 3.839 21.66 3 20.625 3H3.375Z" />
                        <path fill-rule="evenodd" d="m3.087 9 .54 9.176A3 3 0 0 0 6.62 21h10.757a3 3 0 0 0 2.995-2.824L20.913 9H3.087Zm6.163 3.75A.75.75 0 0 1 10 12h4a.75.75 0 0 1 0 1.5h-4a.75.75 0 0 1-.75-.75Z" clip-rule="evenodd" />
                    </svg>
                </button>
                <button class="control-btn" type="button" data-serial-single title="Editar" onclick="event.stopPropagation(); if (typeof btnEditarBienSerial === 'function') btnEditarBienSerial();">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
    ...
    @if(!$serials->isEmpty() && Auth::user()->isAdministrator())
        <div class="serial-select-toolbar">
            <button type="button" class="select-all-btn" id="selectAllSerials"
                    onclick="event.stopPropagation(); toggleSelectAllItems('#serialsGrid');">
                <i class="fas fa-check-double"></i>
                <span data-select-all-label>Seleccionar todos</span>
            </button>
            <span class="serial-select-hint">
                Selecciona dos o más bienes para registrarles un mantenimiento en conjunto.
            </span>
        </div>
    @endif
    ...
    @else
        {{--
            `data-multi-select`: en esta vista se pueden marcar varias tarjetas
            a la vez con la selección normal, para el mantenimiento masivo.
        --}}
        <div class="bienes-grid" id="serialsGrid" data-multi-select>
