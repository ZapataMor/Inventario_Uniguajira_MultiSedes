...
@section('content')
@php
    $isPortalUserCatalog = $isPortalUserCatalog ?? false;
    $usersByScope = $usersByScope ?? collect();
...
<div class="content">
    ...
    <div class="w-full max-w-7xl mx-auto px-4">
        <x-generals.top-bar
            ...
            canCreate="{{ (! $isPortalUserCatalog && Auth::user()->isAdministrator()) ? 'true' : 'false' }}"
        >
            @if($isPortalUserCatalog && Auth::user()->isSuperAdmin())
                <button class="create-btn" onclick="mostrarModal('#modalCrearUsuario')">
                    Crear usuario por sede
                </button>
            @endif
        </x-generals.top-bar>
        ...
        @elseif($isPortalUserCatalog)
            <div class="inventory-sede-list">
                @foreach ($usersByScope as $scopeData)
                    <details class="inventory-sede-dropdown" data-sede-dropdown>
                        <summary class="inventory-sede-summary">
                            <span class="inventory-sede-title">{{ $scopeData['dropdown_label'] }}</span>
                            <span class="inventory-sede-count">
                                <span data-visible-count>{{ $scopeData['users']->count() }}</span> usuarios
                            </span>
                        </summary>

                        <div class="inventory-sede-body w-full">
                            @if($scopeData['users']->isEmpty())
                                <p class="inventory-sede-empty">No hay usuarios registrados en este alcance.</p>
                            @else
                                <div class="w-full overflow-x-auto">
                                    <table class="tabla w-full min-w-full table-auto">
                                        <thead>
                                            <tr class="bg-gray-50 border-b border-gray-200">
                                                <th class="px-4 py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wide w-12">No.</th>
                                                <th class="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wide w-32">Nombre</th>
                                                <th class="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wide w-24">Usuario</th>
                                                <th class="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wide min-w-0 w-full">Email</th>
                                        ...
                                        <tbody>
                                            @foreach($scopeData['users'] as $user)
                                                @php
                                                ...
                                                <tr
                                                    data-user-row
                                                    class="border-t border-gray-100 hover:bg-gray-50 transition-colors duration-100"
                                                    data-search="{{ strtolower($user->name . ' ' . $user->username . ' ' . $user->email) }}"
                                                >
                                                    <td class="px-4 py-3 text-center whitespace-nowrap">
                                                        <div class="text-sm text-gray-700 font-medium">{{ $user->id }}</div>
                                                    </td>
                                                    <td class="px-4 py-3">
                                                    ...
                                                    <td class="px-4 py-3">
                                                        <span class="inline-flex align-items-center px-2.5 py-0.5 rounded-md text-xs font-semibold {{ $isSuperAdmin ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : ($isAdmin ? 'bg-purple-100 text-purple-700 border-purple-200' : 'bg-blue-100 text-blue-700 border-blue-200') }} border">
                                                            {{ $roleLabel }}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td class="px-4 py-3 whitespace-nowrap">
                                                        @if($user->last_login_at)
                                                            <div class="text-sm text-gray-700 font-medium">{{ $user->last_login_at->format('d/m/Y') }}</div>
                                ...
                                <p class="inventory-sede-filter-empty hidden" data-sede-empty>
                                    No hay resultados para este bloque con el filtro actual.
                                </p>
                            @endif
                        </div>
                    </details>
    ...
    @if(Auth::user()->isAdministrator() || ($isPortalUserCatalog && Auth::user()->isSuperAdmin()))
